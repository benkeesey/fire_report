...
class Model(object):
    def __init__(
        ...
        time_window,
        retirement_spend,
        yearly_HSA_qualified_expense,
        balance_brokerage,
        balance_HSA,
        balance_401k,
        balance_roth,
        balance_HSA_qualified_expense,
        balance_401k_contributions,
        growth_comparison_starting_balance,
        HSA_contribution_limit,
        Roth_contribution_limit,
        Traditional_401k_contribution_limit,
        Expense_Ratio,
        Marginal_Tax_Rate,
        Capital_Gains_Tax,
        Retired_Marginal_Tax_Rate,
        Retired_Capital_Gains_Tax,
        **argd
    ):
        """
    ...
    def possible_starting_years(self):
        ...
        end_year = 2021
        num_models_possible = (
            (end_year - self.time_window) - start_year + 2
        )  # Have to add 2 to be inclusive of the end and start year
    ...
    def Backtest_Plot_Spending(self, yearly_market_df):
        ...
        for starting_year in self.possible_starting_years():
            ...
            inital_balance_dict = {
                "balance_brokerage": self.balance_brokerage,
                "balance_HSA": self.balance_HSA,
                "balance_401k": self.balance_401k,
                "balance_roth": self.balance_roth,
                "balance_HSA_qualified_expense": self.balance_HSA_qualified_expense,
                "balance_401k_contributions": self.balance_401k_contributions,
            }

            (
                Total_Net_Return_Spending,
                Total_Liquid_Return_Spending,
            ) = self.model_account_returns_with_spending(
                inital_balance_dict, starting_year, yearly_market_df
            )

            total_liquid_assets_df = pd.concat(
                [total_liquid_assets_df, Total_Liquid_Return_Spending]
            )
            total_net_assets_df = pd.concat(
                [total_net_assets_df, Total_Net_Return_Spending]
            )

        return total_net_assets_df, total_liquid_assets_df

    def model_account_returns_with_spending(
        self, inital_balance_dict, starting_year, yearly_market_df
    ):
        """
        ...
        model_return_data = yearly_market_df.loc[
            starting_year : starting_year + self.time_window - 1,
        ...
        if (
            savings_remainder
            * (1 - self.Marginal_Tax_Rate / 100)
            // self.Roth_contribution_limit
            == 0
        ):
            ...
            savings_remainder -= self.Roth_contribution_limit / (
                1 - self.Marginal_Tax_Rate / 100
            )
        ...
        for i, percent in enumerate(model_return_data):
            ...
            if i >= self.retirement_age - self.age:
                ...
                inital_balance_dict["balance_brokerage"] = inital_balance_dict[
                    "balance_brokerage"
                ] * (1 + percent / 100)
                inital_balance_dict["balance_brokerage"] = inital_balance_dict[
                    "balance_brokerage"
                ] * (1 - self.Expense_Ratio / 100)

                inital_balance_dict["balance_HSA"] = inital_balance_dict[
                    "balance_HSA"
                ] * (1 + percent / 100)
                inital_balance_dict["balance_HSA"] = inital_balance_dict[
                    "balance_HSA"
                ...
                inital_balance_dict["balance_401k"] = inital_balance_dict[
                    "balance_401k"
                ] * (1 + percent / 100)
                inital_balance_dict["balance_401k"] = inital_balance_dict[
                    "balance_401k"
                ...
                inital_balance_dict["balance_roth"] = inital_balance_dict[
                    "balance_roth"
                ] * (1 + percent / 100)
                inital_balance_dict["balance_roth"] = inital_balance_dict[
                    "balance_roth"
                ...
                inital_balance_dict["balance_HSA_qualified_expense"] = (
                    inital_balance_dict["balance_HSA_qualified_expense"]
                    + self.yearly_HSA_qualified_expense
                )

                Brokerage_Capital_Gains = (
                    inital_balance_dict["balance_brokerage"] - Brokerage_YB
                )
                ...
                inital_balance_dict["balance_brokerage"] = inital_balance_dict[
                    "balance_brokerage"
                ] - Brokerage_Capital_Gains * (self.Capital_Gains_Tax / 100)

                # Remove retirement spend from Brokerage account first.
                inital_balance_dict["balance_brokerage"] = (
                    inital_balance_dict["balance_brokerage"] - self.retirement_spend
                )
                ...
                inital_balance_dict["balance_HSA"] = (
                    inital_balance_dict["balance_HSA"] - withdraw_remainder
                )
                ...
                if i + self.age + 1 > 59:
                    ...
                    inital_balance_dict["balance_401k"] = inital_balance_dict[
                        ...
                        (
                            withdraw_remainder
                            * (1 + self.Retired_Marginal_Tax_Rate / 100)
                        )
                    )
                ...
                inital_balance_dict["balance_brokerage"] = (
                    inital_balance_dict["balance_brokerage"] + Brokerage_contribution
                ) * (1 + percent / 100)
                inital_balance_dict["balance_brokerage"] = inital_balance_dict[
                    "balance_brokerage"
                ] * (1 - self.Expense_Ratio / 100)
                ...
                inital_balance_dict["balance_HSA"] = (
                    inital_balance_dict["balance_HSA"] + HSA_contribution
                ) * (1 + percent / 100)
                inital_balance_dict["balance_HSA"] = inital_balance_dict[
                    "balance_HSA"
                ] * (1 - self.Expense_Ratio / 100)

                inital_balance_dict["balance_401k"] = (
                    inital_balance_dict["balance_401k"] + Traditional_401k_contribution
                ) * (1 + percent / 100)
                inital_balance_dict["balance_401k"] = inital_balance_dict[
                    "balance_401k"
                ] * (1 - self.Expense_Ratio / 100)

                inital_balance_dict["balance_roth"] = (
                    inital_balance_dict["balance_roth"] + Roth_contribution
                ) * (1 + percent / 100)
                inital_balance_dict["balance_roth"] = inital_balance_dict[
                    "balance_roth"
                ] * (1 - self.Expense_Ratio / 100)
                ...
                inital_balance_dict["balance_401k_contributions"] = (
                    inital_balance_dict["balance_401k_contributions"]
                ...
                inital_balance_dict["balance_brokerage"] = inital_balance_dict[
                    ...
                    self.Capital_Gains_Tax / 100
                )  # Remove the Captial Gains Tax
        ...
        Total_Net_Return_Spending = pd.Series(
            Total_balance_over_time, index=year_range, name=starting_year
        )
        Total_Liquid_Return_Spending = pd.Series(
            Total_Liquid_balance_over_time, index=year_range, name=starting_year
        )
    ...
    def model_brokerage_returns(self, yearly_market_df, starting_year):
        ...
        after_tax_yearly_investment = self.yearly_savings * (
            1 - self.Marginal_Tax_Rate / 100
        )
        ...
        for i, percent in enumerate(model_return_data):
            ...
            if i >= self.retirement_age - self.age:
                ...
                YE_balance = (YB_balance + after_tax_yearly_investment) * (
                    1 + percent / 100
                )
                # Calculate the dollars gained each year that would need to be taxed as capital gains. Assumes yearly selling and reinvestment of gains (conservative assumption)
                Brokerage_Capital_Gains = YE_balance - (
                    YB_balance + after_tax_yearly_investment
                )

            deposit_balance_over_time.append(
                deposit_balance
            )  # Keep track of contribution balance

            if Brokerage_Capital_Gains < 0:
                Brokerage_Capital_Gains = 0

            YE_balance = YE_balance - Brokerage_Capital_Gains * (
                self.Capital_Gains_Tax / 100
            )  # Remove the Captial Gains Tax
            YE_balance = YE_balance * (
                1 - self.Expense_Ratio / 100
            )  # Remove expense ratio from YE balance
        ...
        liquid_balance = pd.Series(
            liquid_balance_over_time, index=year_range, name=starting_year
        )
    ...
    def model_HSA_returns(self, yearly_market_df, starting_year):
        ...
        YB_balance = (
            self.growth_comparison_starting_balance
        )  # Year Begin balance. Starts with just the beginning account value
        liquid_balance = 0
        deposit_balance = self.growth_comparison_starting_balance
    ...
    def model_roth_returns(self, yearly_market_df, starting_year):
        ...
        after_tax_yearly_investment = self.yearly_savings * (
            1 - self.Marginal_Tax_Rate / 100
        )  # Roth Contributions are paid with after tax dollars
        YB_balance = (
            self.growth_comparison_starting_balance
        )  # Year Begin balance. Starts with just the beginning account value
        liquid_balance = 0
    ...
    def model_trad_401k_returns(self, yearly_market_df, starting_year):
        ...
        for i, percent in enumerate(model_return_data):
            ...
            if i + self.age + 1 > 59:
                liquid_capital_gains = (YE_balance - deposit_balance) * (
                    1 - self.Retired_Capital_Gains_Tax / 100
                )
                liquid_deposits = deposit_balance * (
                    1 - self.Retired_Marginal_Tax_Rate / 100
                )
    ...
    def Backtest_Growth_Models(self, account_type, yearly_market_df):
        ...
        if account_type == "HSA":
            for starting_year in self.possible_starting_years():
                net_return, liquid_balance = self.model_HSA_returns(
                    yearly_market_df, starting_year
                )
            ...
            for starting_year in self.possible_starting_years():
                net_return, liquid_balance = self.model_roth_returns(
                    yearly_market_df, starting_year
                )
                # plt.plot(net_return, color=global_color_scheme[color_alt])
                df = pd.concat([df, net_return])
            ...
            for starting_year in self.possible_starting_years():
                net_return, liquid_balance = self.model_trad_401k_returns(
                    yearly_market_df, starting_year
                )
                # plt.plot(net_return, color=global_color_scheme[color_alt])
                df = pd.concat([df, net_return])
            ...
            for starting_year in self.possible_starting_years():
                net_return, liquid_balance = self.model_brokerage_returns(
                    yearly_market_df, starting_year
                )
                # plt.plot(net_return, color=global_color_scheme[color_alt])
                df = pd.concat([df, net_return])
            ...
            raise ValueError(
                'Account type parameter was not one of following strings in list: ["HSA", "Roth", "Trad_401k", "Brokerage"]'
            )
    ...
    def cash_balance(self):
        ...
        deposits_before_retirement = np.arange(
            start=after_tax_yearly_investment,
            stop=after_tax_yearly_investment * (years_until_retirement + 1),
            step=after_tax_yearly_investment,
        )
        if len(deposits_before_retirement) >= self.time_window:
            deposit_balance = deposits_before_retirement[0 : self.time_window]
        else:
            deposit_balance = np.concatenate(
                [
                    deposits_before_retirement,
                    np.full(
                        self.time_window - len(deposits_before_retirement),
                        deposits_before_retirement[-1],
                    ),
                ]
            )
