...
def historical_market_data():
    ...
    market_data_df["Real_Total_Return_Price"] = (
        market_data_df["Real_Total_Return_Price"]
        .str.strip()
        .str.replace(",", "")
        .astype(float)
    ...
    market_data_df["Real_Monthly_Return"] = market_data_df[
        "Real_Total_Return_Price"
    ].diff()
    market_data_df["Real_Monthly_Return_Percentage"] = (
        market_data_df["Real_Monthly_Return"]
        / market_data_df["Real_Total_Return_Price"]
    ) * 100

    yearly_market_df = pd.DataFrame()
    yearly_market_df["start_Real_Price"] = market_data_df.groupby("Year")[
        "Real_Total_Return_Price"
    ].first()
    yearly_market_df["end_Real_Price"] = market_data_df.groupby("Year")[
        "Real_Total_Return_Price"
    ].last()
    yearly_market_df["Real_Return_Percentage"] = (
        (yearly_market_df["end_Real_Price"] - yearly_market_df["start_Real_Price"])
        / yearly_market_df["start_Real_Price"]
    ) * 100
...
def main(user_id=None):
    """
    The main function of the fire_report will call the mysql db or take in the
    input file to gather the required information to create the model, the plots and the dashboard.
    Here is brief description of each of the required parameters:

    # Personal Financial parameters.
    yearly_savings = Yearly Savings you plan to contribute to your accounts.
    NOTE: This is a Gross amount before tax.
    growth_comparison_starting_balance = starting balance used in the
    (apples to apples) comparison of showing growth between accounts and cash investment.
    time_window = time in years you want to model. Years spend contributing
    and withdrawing from accounts is calculated using your current age and
    your retirement age within this time window
    yearly_HSA_qualified_expense = Yearly estimate of qualifing HSA medical
    expenses. These will be treated as dollars you are able to withdraw without penalty from the HSA account. (currently this is just a static yearly value)
    age = The current age of the person
    retirement_age = The age the person plans of retiring. At this age they
    will stop contributing to their accounts and start withdrawing from them instead as spending.
    retirement_spend = Value person plans on spending per year in retirement
    (All calculated values are in Current Real Dollars.)

    balance_brokerage = Currently balance in Brokerage accounts.
    Used in combined Liquid & Net worth Backtesting Plots
    balance_401k = Currently balance in Traditional 401k accounts.
    Used in combined Liquid & Net worth Backtesting Plots
    balance_HSA = Currently balance in HSA accounts. Used in combined
    Liquid & Net worth Backtesting Plots
    balance_roth = Currently balance in Roth 401k accounts.
    Used in combined Liquid & Net worth Backtesting Plots

    balance_HSA_qualified_expense = Currently balance of
    qualified medical expenses in HSA accounts. Used in combined Liquid & Net worth Backtesting Plots
    ...
    user_var_series = pd.read_excel(
        r"Data\input.xlsx", sheet_name="", index_col=1
    ).drop("Unnamed: 0", axis=1)["Value"]
    ...
    user_id = (
        str(user_var_series["name"])
        + str(user_var_series["age"])
        + str(user_var_series["retirement_age"])
    )
    ...
    HSA_models_df, HSA_models_liquid_df = Model_instance.Backtest_Growth_Models(
        "HSA", yearly_market_df
    )
    (
        Brokerage_models_df,
        Brokerage_models_liquid_df,
    ) = Model_instance.Backtest_Growth_Models("Brokerage", yearly_market_df)
    ...
    Visuals_instance.Backtesting_Plot_Growth_Comparison(
        account1=HSA_models_df,
        account1_name="HSA",
        account2=Brokerage_models_df,
        account2_name="Brokerage",
        cash_account=cash_balance_df,
    )

    # Use the inherited method to calculate Total Spending Net and Liquid Assets DataFrames
    (
        Total_Net_Return_Spending_df,
        Total_Liquid_Return_Spending_df,
    ) = Model_instance.Backtest_Plot_Spending(yearly_market_df)

    # Create total Net and Liquid Spending plots and save them to the Assets folder.
    Visuals_instance.spending_fire_plot(
        df=Total_Net_Return_Spending_df,
        file_name=f"Assets/{Visuals_instance.name}'s Net Backtesting Plot",
    )
    Visuals_instance.spending_fire_plot(
        df=Total_Liquid_Return_Spending_df,
        file_name=f"Assets/{Visuals_instance.name}'s Liquid Backtesting Plot",
    )
    ...
    Visuals_instance.Dashboard(
        Total_Liquid_Return_Spending_df, Total_Net_Return_Spending_df
    )
